...
CREATE TABLE pedidos_itens (
  ...
  CONSTRAINT pedidos_pedidos_itens_fk  FOREIGN KEY (pedido_id)   REFERENCES pedidos (pedido_id)   ON DELETE NO ACTION ON UPDATE NO ACTION NOT DEFERRABLE,
  CONSTRAINT produtos_pedidos_itens_fk FOREIGN KEY (produto_id)  REFERENCES produtos (produto_id) ON DELETE NO ACTION ON UPDATE NO ACTION NOT DEFERRABLE,
  CONSTRAINT envios_pedidos_itens_fk   FOREIGN KEY (envio_id)    REFERENCES envios (envio_id)     ON DELETE NO ACTION ON UPDATE NO ACTION NOT DEFERRABLE
);

COMMENT ON TABLE pedidos_itens                  IS 'Informações sobre os itens pedidos em uma loja.';
COMMENT ON COLUMN pedidos_itens.pedido_id       IS 'Identificador único do pedido ao qual o item pertence.';
COMMENT ON COLUMN pedidos_itens.produto_id      IS 'Identificador único do produto associado ao item do pedido.';
COMMENT ON COLUMN pedidos_itens.numero_da_linha IS 'Número da linha do item no pedido.';
COMMENT ON COLUMN pedidos_itens.quantidade      IS 'Quantidade do item solicitada no pedido.';
COMMENT ON COLUMN pedidos_itens.envio_id        IS 'Identificador único do envio associado ao pedido.';

-- Remover a restrição "pedidos_pedido_itens_fk"
ALTER TABLE pedidos_itens DROP CONSTRAINT IF EXISTS pedidos_pedidos_itens_fk;

-- Remover a restrição "produtos_pedido_itens_fk"
ALTER TABLE pedidos_itens DROP CONSTRAINT IF EXISTS produtos_pedidos_itens_fk;

-- Remover a restrição "envios_pedido_itens_fk"
ALTER TABLE pedidos_itens DROP CONSTRAINT IF EXISTS envios_pedidos_itens_fk;
